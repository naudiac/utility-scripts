...
function formatWithTokens(templateStr, highlight = false) {{
    ...
    if (leadState.lender.trim()) {{
        ...
        res = res.replaceAll("[Lender_Or_Default]", `${{lenderToken}} or other daily MCA positions`);
    }} else {{
        res = res.replaceAll("[Lender_Or_Default]", "OnDeck, Rapid, or high-factor MCA advances");
    }}
